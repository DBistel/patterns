...
export function haveEnoughFood(backpack, people) {
  const foodItems = backpack.filter(items => items.category === "food");
  if (people === 0 && foodItems.length === 0) return true;
  if (people === 0) return true;
  if (foodItems.length < people) return false;
  if (foodItems.length > people) return true;
